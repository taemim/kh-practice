...
--1. 영어영문학과(학과코드 002) 학생들의 학번과 이름,
-- 입학 년도를 입학 년도가 빠른 순으로 표시하는 SQL 문장을 작성하시오.
...
--(단, 교수 중 2000년 이후 출생자는 없으며 
--출력 헤더는 "교수이름", "나이"로 한다. 나이는 ‘만’으로 계산한다.)
SELECT
       PROFESSOR_NAME 교수이름 
     , CASE
       WHEN TO_CHAR(SUBSTR(PROFESSOR_SSN,1,2),'99') >= 50 
       THEN FLOOR(MONTHS_BETWEEN(SYSDATE, TO_DATE(SUBSTR(PROFESSOR_SSN, 1, 6), 'RRMMDD')) / 12) 
       WHEN TO_CHAR(SUBSTR(PROFESSOR_SSN,1,2),'99') < 50
       THEN FLOOR(MONTHS_BETWEEN(SYSDATE,TO_DATE(CONCAT('19',SUBSTR(PROFESSOR_SSN,1,6)),'YYYYMMDD'))/12)
       END AS 나이
  FROM TB_PROFESSOR
...
-- TO_DATE('49/10/11','YY/MM/DD') 은 각각 몇 년 몇 월 몇 일을 의미할까? 
-- 또 TO_DATE('99/10/11','RR/MM/DD')
-- TO_DATE('49/10/11','RR/MM/DD') 은 각각 몇 년 몇 월 몇 일을 의미할까?
SELECT
     TO_CHAR(TO_DATE('99/10/11'),'YYYY"년" MM"월" DD"일"') AS  "99/10/11"
    , TO_CHAR(TO_DATE('49/10/11'),'YYYY"년" MM"월" DD"일"') AS "49/10/11"
    , TO_CHAR(TO_DATE('99/10/11'),'RRRR"년" MM"월" DD"일"') AS "99/10/11"
    , TO_CHAR(TO_DATE('49/10/11'),'RRRR"년" MM"월" DD"일"') AS "49/10/11" 
FROM DUAL;
...
-- 단, 이때 출력 화면의 헤더는 "평점" 이라고 찍히게 하고, 점수는 반올림하여 소수점 이하 한 자리까지만 표시핚다.
SELECT 
